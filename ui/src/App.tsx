...
export function App() {
  ...
  const newChat=(path?:string)=>{setWorkspace(path||current?.session.Workdir||workspace);select("");};
  ...
  return <main className={`app-shell ${inspector?"with-inspector":""}`}><aside className="sidebar">{sidebar}</aside><section className="workspace"><header className="workspace-header"><div className="workspace-title"><button className="quiet mobile-sessions" onClick={()=>setDrawer(true)} aria-label="Open sessions"><Icon name="menu"/></button><div><small>WORKSPACE</small><strong title={current?.session.Workdir||workspace}>{(current?.session.Workdir||workspace).split("/").filter(Boolean).at(-1)||"Choose a workspace"}</strong></div></div><div className="header-actions"><span className="connection" role="status"><i className={connection==="Connected"||!id?"connected":""}/>{id?connection:"Local"}</span><button className="quiet commands-button" onClick={()=>setPalette(true)}>Commands <kbd>⌘ K</kbd></button><button className={`secondary ${inspector?"selected":""}`} onClick={()=>setInspector(v=>!v)} aria-expanded={inspector}>Inspect</button></div></header><div className="session-heading"><div><h1>{current?.session.Label||current?.session.Task||"New conversation"}</h1><p>{current?.session.Model||config?.model||"Loading model…"} <span>· {effectiveMode} mode</span> {id&&<span>· {current?.session.Status||"loading"}</span>}</p></div>{current?.can_delete&&<button className="quiet" onClick={()=>setDeleting(true)}>Delete chat</button>}{current?.can_rename&&<button className="quiet" onClick={()=>{setLabel(current.session.Label||"");setRename(true);}}>Rename</button>}</div>
    {!config?.ready&&config?.read_only_reason&&<div className="setup-notice">{config.read_only_reason}</div>}
    {!config?.ready&&config&&!config.read_only_reason&&<div className="setup-notice"><strong>Connect a model to start coding</strong><p>Run <code>mulch config</code> in your terminal to save your provider, model, and API key, then restart <code>mulch web</code>. Saved conversations remain available here.</p></div>}
