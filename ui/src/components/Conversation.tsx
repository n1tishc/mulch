...
function CodeBlock({children}:{children:React.ReactNode}) {
  ...
  const node=Array.isArray(children)?children[0]:children, language=typeof node==="object"&&node&&"props" in node?String((node as React.ReactElement<{className?:string}>).props.className||"").replace("language-",""):"";
  return <div className="code-block"><div className="code-toolbar"><span>{language||"code"}</span><button className="quiet copy" onClick={()=>void navigator.clipboard.writeText(ref.current?.textContent||"").then(()=>setCopied(true))}>{copied?"Copied":"Copy code"}</button></div><pre ref={ref}>{children}</pre></div>;
}
const Message = memo(function Message({row,onInspect}:{row:ConversationRow;onInspect:(e:TraceEvent)=>void}) {
  ...
  const label=row.kind==="user"?"You":"Mulch";
  return <article className={`message ${row.kind}`} aria-label={`${label} message`}><div className="message-rail"><span className="message-avatar" aria-hidden="true">{row.kind==="user"?"Y":"m"}</span><div className="message-label">{label}<button className="quiet event-link" onClick={()=>onInspect(row.event)} aria-label={`Inspect ${label} message event ${row.event.seq}`}>#{row.event.seq}</button></div></div><div className="message-body"><div className="prose"><Markdown skipHtml remarkPlugins={[remarkGfm]} components={{img:({alt})=><span>[Image: {alt || "omitted"}]</span>,pre:({children})=><CodeBlock>{children}</CodeBlock>,code:({children,className})=><code className={className}>{children}</code>,a:({children,...props})=><a {...props} target="_blank" rel="noreferrer">{children}</a>}}>{row.text.slice(0,50000)}</Markdown></div>{row.kind==="assistant" && <Copy text={row.text}/>}</div></article>;
});
