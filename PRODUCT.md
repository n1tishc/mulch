...
Provider secrets stay in the backend. Configuration comes from launch flags/environment. The browser shows effective non-secret configuration and historical run metadata; old missing evidence remains unknown. Shared event history supports read-only terminal inspection through `/inspect` without transferring execution ownership.
